...
def test_plot_dgs_plotly(tmp_path, fe_map):
    ...
    _ = plotting.plot_DGs(fe_map, source="MLE", filename=output_file.as_posix(), plotly=True)
    assert output_file.exists()
...
def test_plot_all_ddgs_plotly(tmp_path, fe_map):
    ...
    fe_map.generate_absolute_values()
    _ = plotting.plot_all_DDGs(fe_map, source="MLE", filename=output_file.as_posix(), plotly=True)
    assert output_file.exists()
